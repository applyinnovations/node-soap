      ...
      if (!objectName && top.name === "Body" && name !== "Fault") {
        ...
        if (!message) {
          try {
            ...
            this.definitions.messages[originalName] =
              this.definitions.messages[name];
          } catch (e) {
    ...
    if (Array.isArray(obj)) {
      ...
      for (name in obj) {
        ...
        if (isFirst) {
          ...
          if (this.definitions.schemas) {
            if (schema) {
              const childSchemaObject = this.findChildSchemaObject(
                schemaObject,
                name
              );
  ...
  private _processNextInclude(includes: elements.IInclude[], callback) {
    ...
    if (!/^https?:/i.test(this.uri) && !/^https?:/i.test(include.location)) {
      ...
      if (isFixed) {
        includePath = path.resolve(
          ...
          path.parse(include.location).base
        );
      } else {
    ...
    open_wsdl_recursive(includePath, options, (err, wsdl) => {
      ...
      if (wsdl.definitions instanceof elements.DefinitionsElement) {
        ...
        return callback(
          new Error(
            "wsdl.defintions is not an instance of elements.DefinitionsElement"
          )
        );
      }
      ...
      if (top) {
        try {
          ...
          if (this.options.strict) {
            ...
            stack.push(
              new elements.Element(nsName, attrs, options, schemaAttrs)
            );
          }
        }
      } else {
