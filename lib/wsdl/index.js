...
var WSDL = /** @class */ (function () {
    ...
    WSDL.prototype.xmlToObject = function (xml, callback) {
        ...
        p.onopentag = function (node) {
            ...
            if (!objectName && top.name === "Body" && name !== "Fault") {
                ...
                if (!message) {
                    try {
                        ...
                        _this.definitions.messages[originalName] =
                            _this.definitions.messages[name];
                    }
                    catch (e) {
            ...
            for (var prefix in xmlns) {
                if (xmlns[prefix] === XSI_URI &&
                    prefix + ":type" in elementAttributes) {
        ...
        p.ontext = function (text) {
            ...
            var value;
            if (_this.options &&
                _this.options.customDeserializer &&
                _this.options.customDeserializer[name]) {
    ...
    WSDL.prototype.objectToXML = function (obj, name, nsPrefix, nsURI, isFirst, xmlnsAttr, schemaObject, nsContext) {
        ...
        var xmlnsAttrib = "";
        if (nsURI && isFirst) {
            if (this.options.overrideRootElement &&
                this.options.overrideRootElement.xmlnsAttributes) {
        ...
        if (xmlnsAttr &&
            !(this.options.overrideRootElement &&
                this.options.overrideRootElement.xmlnsAttributes)) {
                        ...
                        if (schema) {
                            ...
                            if (childSchemaObject &&
                                    ...
                                    if (elementQName.prefix === utils_1.TNS_PREFIX) {
                                        ...
                                        childNsURI =
                                            schema.xmlns[childNsPrefix] ||
                                                this.definitions.xmlns[childNsPrefix];
                                    }
                                    var unqualified = false;
                                    // Check qualification form for local elements
                                    if (childSchemaObject.$name &&
                                ...
                                if (childSchemaObject.$type) {
                                    ...
                                    var typeURI = schema.xmlns[typePrefix] ||
                                        this.definitions.xmlns[typePrefix];
                                    childNsURI = typeURI;
                                    if (typeURI !== "http://www.w3.org/2001/XMLSchema" &&
                                        typePrefix !== utils_1.TNS_PREFIX) {
                                        // Add the prefix/namespace mapping, but not declare it
                                        nsContext.addNamespace(typePrefix, typeURI);
                                    }
                                    resolvedChildSchemaObject =
                                        this.findSchemaType(typeQName.name, typeURI) ||
                                            childSchemaObject;
                                }
                                else {
                                    resolvedChildSchemaObject =
                                        this.findSchemaObject(childNsURI, childName) ||
                                            childSchemaObject;
                                }
                                if (childSchemaObject.$baseNameSpace &&
                                ...
                                if (Array.isArray(child)) {
                                    ...
                                    childXmlnsAttrib =
                                        childXmlnsAttrib && childXmlnsAttrib.length
                                            ? childXmlnsAttrib
                                            : currentChildXmlnsAttrib;
                                }
                                else {
                            ...
                            else if (obj[this.options.attributesKey] &&
    ...
    WSDL.prototype.findChildSchemaObject = function (parameterTypeObj, childName, backtrace) {
        ...
        var child;
        var ref;
        if (Array.isArray(parameterTypeObj.$lookupTypes) &&
        ...
        if (object.$base &&
            (!Array.isArray(object.children) || !object.children.length)) {
                ...
                if (child.$base) {
                    ...
                    childNsURI =
                        child.xmlns[baseQName.prefix] ||
                            child.schemaXmlns[baseQName.prefix];
    ...
    WSDL.prototype._processNextInclude = function (includes, callback) {
        ...
        if (!/^https?:/i.test(this.uri) && !/^https?:/i.test(include.location)) {
            var isFixed = this.options.wsdl_options !== undefined &&
        ...
        if (this.options.wsdl_options !== undefined &&
            typeof this.options.wsdl_options.overrideImportLocation === "function") {
            includePath =
                this.options.wsdl_options.overrideImportLocation(includePath);
        }
        var options = Object.assign({}, this.options);
        // follow supplied ignoredNamespaces option
        options.ignoredNamespaces =
            this._originalIgnoredNamespaces || this.options.ignoredNamespaces;
    ...
    WSDL.prototype._parse = function (xml) {
        ...
        p.onopentag = function (node) {
            ...
            if (top) {
                try {
                    top.startElement(stack, nsName, attrs, options, schemaAttrs, attrs === null || attrs === void 0 ? void 0 : attrs.ref);
    ...
    if (!/^https?:/i.test(uri)) {
        ...
        httpClient.request(uri, null /* options */, function (err, response, definition) {
            if (err) {
                ...
                callback(new Error("Invalid WSDL URL: " +
                    uri +
                    "\n\n\r Code: " +
                    response.status +
                    "\n\n\r Response Body: " +
